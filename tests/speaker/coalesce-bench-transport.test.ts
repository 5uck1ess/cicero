...
test("a response that never closes is abandoned at the deadline", async () => {
  ...
  const speak = createBenchSpeaker("http://tts.invalid/v1/audio/speech", {}, 150, undefined, fetcher);
...
test("a body larger than the audio bound is refused instead of read whole", async () => {
  ...
  const fetcher = async (): Promise<Response> =>
    new Response(new Uint8Array(4_096), { headers: { "Content-Type": "audio/wav" } });
  const speak = createBenchSpeaker("http://tts.invalid/v1/audio/speech", {}, 30_000, 512, fetcher);
...
test("the default bound is the provider audio limit", async () => {
  ...
  const speak = createBenchSpeaker("http://tts.invalid/v1/audio/speech", {}, 30_000, undefined, fetcher);
...
test("an error body is bounded instead of read whole", async () => {
  const fetcher = async (): Promise<Response> =>
    new Response("upstream exploded: ".padEnd(200_000, "x"), { status: 500 });
  const speak = createBenchSpeaker("http://tts.invalid/v1/audio/speech", {}, 30_000, undefined, fetcher);
  const failure = await speak("Warming up.").then(() => null, (error: unknown) => error as Error);
