...
- **The switchboard runs before the brain.** Transfers ("talk to the coder"), roll call, "details", and user-defined quick intents have immediate lexical fast paths. Switchboard misses classify concurrently with safe brains, holding output and ACP permissions until the decision (1500 ms default deadline). Unsafe routes classify first; see [intent routing](office.md#intent-routing).
