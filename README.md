...
- **Route in ~30 ms, fully local (opt-in)** — a fine-tuned [Laya sidecar](sidecars/laya-switchboard/README.md) can replace the LLM intent prompt for transfers, roll call, standups and call-me. **Bring your own fine-tuned checkpoint for now**; the base model doesn't route zero-shot. A public synthetic-data checkpoint and the training recipe are coming.
