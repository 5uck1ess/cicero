...
export interface StreamCandidate {
  ...
  path?: string; language?: string; chunkMs?: number; pace?: "realtime" | "fast";
}
 ...
 */
export class ChunkedResponseReader {
  private state:
...
export function responseLimits(overrides: Partial<LiveStreamResponseLimits> = {}): LiveStreamResponseLimits {
...
export async function transcribeLive(
  ...
  const text = finalText as string;
  if (!text.trim()) {
...
/** One live microphone upload. The same chunked response framer and SSE event
 * names used by the paced benchmark above are used for actual capture. */
export interface LivePcmSession {
...
export interface OpenLivePcmOptions {
  host: string;
  port: number;
  model: string;
  sampleRate: number;
...
const MAX_LIVE_PCM_BYTES = 4 * 1024 * 1024;
const MAX_LIVE_PARTIAL_CHARS = 16_384;
const MAX_LIVE_EVENT_BYTES = 64 * 1024;
const MAX_LIVE_BODY_BYTES = 512 * 1024;
const MAX_LIVE_PARTIAL_QUEUE = 16;

export function openLivePcm(options: OpenLivePcmOptions): LivePcmSession {
  if (!options.host || options.host.length > 255 || /[\s\/?#@\\]/.test(options.host))
    throw new RangeError("invalid live STT host");
  if (!Number.isSafeInteger(options.port) || options.port < 1 || options.port > 65_535)
    throw new RangeError("invalid live STT port");
  ...
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 300_000)
    throw new RangeError("invalid live PCM deadline");
  const now = options.now ?? (() => performance.now());
  const connect = options.connect ?? ((value) => Bun.connect(value));
  const reader = new ChunkedResponseReader(responseLimits({
    maxChunkBytes: MAX_LIVE_BODY_BYTES,
    maxEventBytes: MAX_LIVE_EVENT_BYTES,
  ...
  let transcript = "";
  let finalText: string | null = null;
  let drain: { resolve: () => void; reject: (reason: Error) => void } | null = null;
  ...
  const stop = (reason?: Error): void => {
    ...
    if (socket) {
      try { socket.terminate(); releaseConfirmed = true; }
      catch {
    ...
    if (reason) {
      ...
      waiter?.reject(new Error("live stream complete"));
      resolveFinal(finalText!.trim());
    }
    partialWake?.(); partialWake = null;
  };
  const fail = (reason: Error): void => stop(reason);
  const onAbort = (): void => fail(new DOMException("live transcription aborted", "AbortError"));
  const timer = setTimeout(() => fail(new Error("live transcription deadline exceeded")), timeoutMs);
  ...
  const consumeEvent = (block: string): void => {
    ...
    if (event.type === "transcript.text.delta") {
      ...
      if (transcript.length + event.delta.length > MAX_LIVE_PARTIAL_CHARS)
        throw new RangeError("live transcript exceeds character limit");
      transcript += event.delta;
      if (partialQueue.length === MAX_LIVE_PARTIAL_QUEUE) partialQueue.shift();
      partialQueue.push(transcript);
      options.onPartial?.(transcript, now());
      partialWake?.(); partialWake = null;
    } else if (event.type === "transcript.text.done") {
      if (typeof event.text !== "string" || event.text.length > MAX_LIVE_PARTIAL_CHARS)
        throw new RangeError("invalid live final transcript");
      finalText = event.text;
    }
  };
  const consume = (fragment: string): void => {
    eventBuffer += fragment;
    let match: RegExpExecArray | null;
    while ((match = /\r?\n\r?\n/.exec(eventBuffer))) {
      const block = eventBuffer.slice(0, match.index);
      if (new TextEncoder().encode(block).length > MAX_LIVE_EVENT_BYTES)
        throw new RangeError("live SSE event exceeds byte limit");
      consumeEvent(block);
      eventBuffer = eventBuffer.slice(match.index + match[0].length);
    }
    if (new TextEncoder().encode(eventBuffer).length > MAX_LIVE_EVENT_BYTES)
      throw new RangeError("live SSE event exceeds byte limit");
  };
  const writeAll = async (data: Uint8Array | string): Promise<void> => {
    let bytes = typeof data === "string" ? new TextEncoder().encode(data) : data;
    while (bytes.length) {
      if (closed || !socket) throw new Error("live transcription socket closed");
      const written = socket.write(bytes);
      if (written >= bytes.length) return;
      bytes = bytes.subarray(Math.max(0, written));
      await new Promise<void>((resolve, reject) => { drain = { resolve, reject }; });
    }
  };
  const query = new URLSearchParams({ model: options.model, sample_rate: String(options.sampleRate), channels: "1", sample_format: "s16le" });
    ...
    const pending = connect({
      ...
      socket: {
        data: (_socket, chunk) => {
          ...
          } catch (error) { fail(error instanceof Error ? error : new Error("invalid live response")); }
        },
        drain: () => { const waiter = drain; drain = null; waiter?.resolve(); },
        close: () => { socketClosed = true; releaseConfirmed = true; if (!closed) fail(new Error("live transcription socket closed early")); },
        error: () => fail(new Error("live transcription socket error")),
      },
    });
  ...
  return {
    ...
    push(pcm) {
      if (closed || ended) throw new Error("live stream is closed");
      if (!(pcm instanceof Uint8Array) || pcm.length === 0 || pcm.length > 64 * 1024 || pcm.length % 2)
        throw new RangeError("invalid live PCM chunk");
      if (receivedBytes + pcm.length > MAX_LIVE_PCM_BYTES) throw new RangeError("live PCM exceeds byte limit");
      receivedBytes += pcm.length;
      const copy = pcm.slice();
      work = work.then(async () => {
        await writeAll(`${copy.length.toString(16)}\r\n`);
        await writeAll(copy);
        await writeAll("\r\n");
      }).catch((error) => fail(error instanceof Error ? error : new Error("live write failed")));
    },
    end() {
      if (!ended && !closed) {
        ended = true;
        work = work.then(async () => {
          ...
          requestEnded = true;
          maybeFinish();
        }).catch((error) => fail(error instanceof Error ? error : new Error("live end failed")));
      }
      return final;
    },
    abort() { onAbort(); },
    ...
    partials: {
      async *[Symbol.asyncIterator]() {
        while (!closed || partialQueue.length) {
